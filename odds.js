// ─── ODDS PROXY — Netlify serverless function ───────────────────────────────
// Uses Node's built-in https module (no fetch, works on all Node versions).
// Your API key lives ONLY here on the server — never in the browser.
// ─────────────────────────────────────────────────────────────────────────────
...
exports.handler = async function () {
  ...
  if (!API_KEY) {
    return respond({ live: false, reason: "no_key",
      message: "No ODDS_API_KEY found. Add it in Netlify → Project configuration → Environment variables, then redeploy.", matches: [] });
  }
...
// ── shape one Odds-API game into what the UI expects ────────────────────────
